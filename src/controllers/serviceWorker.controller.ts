...
export const serviceWorker = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!servicesWorkers) return res.status(404).json({ error: 'serviceWorkers not found' })
    const workersProfessions = await prisma.profession.findMany()
    const newWorkers = servicesWorkers.map((serviceWorker) => {
      return {
        ...serviceWorker,
        profession: workersProfessions.find(profession => profession.id === serviceWorker.professionId),
      }
    })
